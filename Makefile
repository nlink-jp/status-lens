APP_NAME    := status-lens
NAME        := status-lens
BUNDLE_ID   := jp.nlink.status-lens
...
# Hardened Runtime alone suffices. The single binary is the GUI; it responds
# to --version only (no CLI subcommands, by design — see RFP).
...
BREW_KIND      := cask
BREW_DESC      := Menu-bar service status watcher for Statuspage-hosted pages
BREW_NAME      := $(NAME)
